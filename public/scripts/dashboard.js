...
function timeHandler() {
  ...
  timer = window.setInterval(() => {
    ...
    tempo_integral.textContent = `${zeroFill(hour, 2)}:${zeroFill(
      min,
      2
    )}:${zeroFill(s, 2)}`;
    tempo_andando.textContent = `${zeroFill(
      Math.trunc(segundos_andando / 3600),
      2
    )}:${zeroFill(Math.trunc((segundos_andando % 3600) / 60), 2)}:${zeroFill(
      (segundos_andando % 3600) % 60,
      2
    )}`;
    tempo_parado.textContent = `${zeroFill(
      Math.trunc(segundos_parado / 3600),
      2
    )}:${zeroFill(Math.trunc((segundos_parado % 3600) / 60), 2)}:${zeroFill(
      Math.trunc((segundos_parado % 3600) % 60),
      2
    )}`;
    s += 1;
  }, 1000);
...
function handleData(data) {
  ...
  // médias
  data_counter += 1;
  max_sum.vel += Number(data.vel);
  max_sum.rpm += Number(data.rpm);
  max_sum.temp_cvt += Number(data.temp_cvt);
...
function voltasHandler() {
  ...
  // 00:00:00
  tempo_medio.textContent =
    `${zeroFill(Math.trunc(tempoMedioSegundos / 3600), 2)}:${zeroFill(
      Math.trunc((tempoMedioSegundos % 3600) / 60),
      2
    )}` + `:${zeroFill(Math.trunc((tempoMedioSegundos % 3600) % 60), 2)}`;
  tempo_ultima_volta.textContent =
    `${zeroFill(Math.trunc(tempoUltimaVoltaSegundos / 3600), 2)}:${zeroFill(
      Math.trunc((tempoUltimaVoltaSegundos % 3600) / 60),
      2
    )}` + `:${zeroFill(Math.trunc((tempoUltimaVoltaSegundos % 3600) % 60), 2)}`;
  tempo_melhor_volta.textContent =
    `${zeroFill(Math.trunc(tempoMelhorVoltaSegundos / 3600), 2)}:${zeroFill(
      Math.trunc((tempoMelhorVoltaSegundos % 3600) / 60),
      2
